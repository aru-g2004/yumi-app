...
const LiveDesignLab: React.FC<LiveDesignLabProps> = ({ onEarn }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in zoom-in duration-700">
      ...
      <div className="bg-stone-900 rounded-[4rem] p-16 text-white relative overflow-hidden shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          <div className="absolute top-0 right-0">
            <button
              ...
              className="bg-amber-400 text-amber-950 px-6 py-3 rounded-full font-black text-xs uppercase tracking-widest hover:scale-105 transition-transform shadow-lg flex items-center gap-2"
            >
              <Sparkles className="w-4 h-4" /> Lucky Spin
            </button>
          </div>
          <div className="bg-stone-800 p-5 rounded-[2rem] mb-8">
            <Mic className="w-10 h-10 text-emerald-400" />
          </div>
          <h2 className="text-5xl font-black mb-6 tracking-tighter">The Design Lab.</h2>
          <p className="text-stone-400 max-w-lg text-lg font-light leading-relaxed mb-12">
          ...
          {!isActive ? (
            <button
              ...
              className={`bg-white text-stone-900 px-12 py-6 rounded-3xl font-black text-lg tracking-widest uppercase hover:bg-stone-50 transition-all flex items-center gap-3 shadow-2xl disabled:opacity-50 ${pendingReward ? 'opacity-20 pointer-events-none' : ''}`}
            >
              {isConnecting ? <Loader2 className="w-6 h-6 animate-spin" /> : <Sparkle className="w-5 h-5 fill-stone-900" />}
              {isConnecting ? 'Initializing...' : 'Enter Lab'}
            </button>
          ) : (
            <button
              onClick={stopSession}
              className="bg-rose-500 text-white px-12 py-6 rounded-3xl font-black text-lg tracking-widest uppercase hover:bg-rose-600 transition-all flex items-center gap-3 shadow-2xl"
            >
      ...
      {isActive && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 animate-in slide-in-from-bottom-12 duration-700">
          <div className="md:col-span-2 bg-white rounded-[3rem] p-10 shadow-xl border border-stone-50">
            <h3 className="text-[10px] font-black text-stone-300 uppercase tracking-[0.3em] mb-8 flex items-center gap-2">
              <Volume2 className="w-4 h-4" /> Feedback Stream
            </h3>
            <div className="space-y-6">
              {transcription.length === 0 ? (
                <p className="text-stone-300 italic font-light">"Describe your vision for a new series..."</p>
              ) : (
                transcription.map((t, i) => (
                  <div key={i} className={`p-6 rounded-[2rem] transition-all ${i === 0 ? 'bg-stone-50 text-stone-900 font-medium' : 'text-stone-300 text-sm opacity-50'}`}>
                    {t}
                  </div>
                ))
              )}
          ...
          <div className="bg-white rounded-[3rem] p-10 shadow-xl border border-stone-50 flex flex-col items-center justify-center text-center relative transition-all duration-500">
            {pendingReward ? (
              <div className="space-y-6 animate-in zoom-in duration-500 w-full">
                <div className="bg-emerald-50 p-6 rounded-full inline-block animate-bounce">
                  <Coins className="w-10 h-10 text-emerald-600" />
                </div>
                <div>
                  <h4 className="text-[10px] font-black uppercase tracking-widest text-stone-400 mb-2">Curator's Critique</h4>
                  <p className="font-medium text-lg italic text-stone-800 leading-relaxed">"{pendingReward.reason}"</p>
                </div>
                <div className="pt-2">
              ...
              <div className="animate-in zoom-in duration-500">
                <div className="bg-emerald-100 p-6 rounded-full mb-6 inline-block">
                  <Coins className="w-12 h-12 text-emerald-600" />
                </div>
                <p className="text-2xl font-black text-emerald-600 tracking-tight">{successMsg}</p>
              </div>
            ) : (
              <>
                <Sparkles className="w-12 h-12 text-stone-100 mb-6" />
                <p className="text-stone-400 font-light px-4">Impress the curator with chic design concepts to unlock budget.</p>
              </>
            )}
          </div>
        </div>
      ...
      <div className="bg-stone-50 p-8 rounded-[3rem] border border-stone-100 flex items-start gap-6 text-stone-500">
        <Sparkles className="w-6 h-6 flex-shrink-0 text-rose-300" />
        <div className="text-sm font-light leading-relaxed">
          <p className="font-bold mb-2 text-stone-800">Curation Logic</p>
