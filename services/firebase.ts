...
/**
 * Uploads a base64 video to Firebase Storage and returns the download URL
 * Path: users/{userId}/themes/{themeId}/{filename}
 */
export const uploadVideoToStorage = async (
    ...
    const currentUser = auth.currentUser;
    if (!currentUser) {
        throw new Error('User must be authenticated to upload videos to Firebase Storage');
    }

    if (currentUser.uid !== userId) {
        throw new Error('Cannot upload videos for another user');
    }

    const storagePath = `users/${userId}/themes/${themeId}/${filename}`;
    console.log('[Firebase Storage] Uploading video to:', storagePath);
    ...
    console.log('[Firebase Storage] Video Blob size:', blob.size, 'bytes');

    const storageRef = ref(storage, storagePath);

    try {
        const uploadResult = await uploadBytes(storageRef, blob);
        console.log('[Firebase Storage] Video upload successful:', uploadResult.metadata.fullPath);
        const downloadURL = await getDownloadURL(storageRef);
        return downloadURL;
    } catch (error: any) {
        console.error('[Firebase Storage] Video upload failed:', error);
        throw new Error(`Firebase Storage video upload failed: ${error.message}`);
    }
};
